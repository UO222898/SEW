...
def obtenerPuntos(nombreXML, cotaMaxAltura):
    ...
    puntos = "0,"+str(cotaMaxAltura)
    distancia = 0.

    # Recorrido de los tramos del árbol
    for hijo in raiz.findall('.//{http://www.uniovi.es}tramos/*'): # Expresión Path
        altitud = cotaMaxAltura - float(hijo.find("./{http://www.uniovi.es}coordenada").get("altitud"))
        distancia= round(distancia + float(hijo.find("./{http://www.uniovi.es}distancia").text),2)
        puntos += "\t\t"+str(distancia)+","+str(altitud)+"\n"

    puntosLista = puntos.split('\n')

    pInicio = puntosLista[0].lstrip()
    pFin = puntosLista[len(puntosLista)-2].lstrip()

    puntos += ""+pFin.split(",")[0]+","+str(cotaMaxAltura)+"\n"
    puntos += pInicio
